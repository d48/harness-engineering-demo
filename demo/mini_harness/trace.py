"""Terminal trace rendering: makes the agent loop legible to an audience."""
...
class Trace:
    """Prints the anatomy of each loop turn: thought -> tool call -> observation."""
    ...
    def turn(self, n: int) -> None:
        self._emit()
        self._emit(f"{self._prefix()} {paint(f'turn {n}', BOLD)}")
